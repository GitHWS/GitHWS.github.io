...
나의 경우 CRA 프로젝트을 완료하고 `JavaScript`에서 `TypeScript`로 적용하는 과정에서 이러한 경고 메세지를 볼 수 있었다.

기존 프로젝트에 TypeScript를 적용하는 것은 처음 해보기 때문에 이런 경고 메세지가 쭉 출력될지 몰랐기 때문에 좋다고 `npm i typescript`을 입력해버린 것이다.
